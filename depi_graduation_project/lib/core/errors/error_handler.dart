...
class ErrorHandler {
  static ServerFailure handle(Object error, [StackTrace? stackTrace]) {
    if (error is DioException) {
      final failure = ServerFailure.fromDioExcepiton(error);
      AppLogger.error(
        '🌐 Dio Error: ${failure.errMessage} (${failure.statusCode})',
      );
      return failure;
    } else if (error is SocketException) {
      AppLogger.error('📡 Network Error: No Internet Connection');
      return ServerFailure(errMessage: 'No Internet Connection', statusCode: 0);
    } else if (error is CustomException) {
      AppLogger.error(
        '💾 Backend Error: ${error.message} (${error.statusCode})',
      );
